...
void makeVisit(point region) {
    ...
    queue<point> que;
    que.push(region);   tempVector.push_back(region);
    ...
    while(!que.empty()) {
        ...
        for(int i = 0; i < 4; ++i) {
            ...
            que.push(next); tempVector.push_back(next);
...
int BFS(const point &region) {
    ...
    while(!que.empty()) {
        ...
        for(int i = 0; i < 4; ++i) {
            ...
            if((table[next.y][next.x] != 0) && (table[next.y][next.x] != table[region.y][region.x])) return distance - 1;
