...
ElementType HEAP_Delete(Heap* heap) {
    ...
    while (curr * 2 < heap->used_size) {
        int max_child = get_max_child(heap, curr * 2, curr * 2 + 1);
        if (nodes[curr].Data < nodes[max_child].Data) {
            swap(&nodes[curr], &nodes[max_child]);
            curr = max_child;
        }
        else {
            break;
...
int main(void) {
    ...
    maxHeap = HEAP_Create(Capacity + 1);
    for (int i = 0; i < N; ++i) {
        (void)scanf("%d", &temp);
        HEAP_Insert(maxHeap, temp);
    }
    HEAP_PrintNodes(maxHeap);
    HEAP_Delete(maxHeap);
    HEAP_PrintNodes(maxHeap);
